...
def conversion_section_cases():
    ...
    # ── 5차 회귀 단언 ─────────────────────────────────────────────────────
    print("\n  [5-5] 5차 — 머리행 판별·문서 집합·배치")
    import handoff5 as _h5
    _N = docparse.normalize_for_match

    def _tbl(*rows):
        return {"rows": [[{"text": c} for c in r] for r in rows]}

    # (1) 겸직 표 머리행은 **조합**으로 판별한다. 실측에서 같은 「대상회사」가 진짜
    #     겸직 표와 오탐 양쪽에 쓰였다 — 낱말 하나로는 갈리지 않는다.
    for _name, _t, _want in [
        ("KB금융·iM·KB국민카드",
         _tbl(["성 명", "대상 계열사", "직 책", "선임시기", "비 고"],
              ["송호영", "KB캐피탈", "소비자보호본부장", "2023.1월", "상근"]), "대상 계열사"),
        ("우리금융지주",
         _tbl(["성명", "대상회사", "직위", "겸임일자", "상근/비상근 여부"],
              ["손태승", "우리은행", "은행장", "2019.01.11", "상근"]), "대상회사"),
        ("신한지주",
         _tbl(["성명", "대상회사", "직위명", "선임일", "비고"],
              ["정상혁", "신한은행", "은행장", "2023.02.15", "상근"]), "대상회사"),
        ("메리츠금융지주(2단 머리행)",
         _tbl(["겸직대상자", "계열회사 겸직내역", "비 고"],
              ["", "회사명", "직 책", "상근여부"]), "계열회사 겸직내역"),
        ("ABL·동양생명(2단 머리행)",
         _tbl(["당사", "겸임회사"], ["성명", "직위", "상근/비상근 여부", "겸임일자",
                                     "회사명", "직위"]), "겸임회사"),
        ("한국금융지주",
         _tbl(["직명", "성명", "회사명", "직책", "상근여부"],
              ["대표이사", "김남구", "한국투자증권(주)", "회장", "상근"]), "회사명"),
        ("하나은행(구)",
         _tbl(["성명", "회사명", "직책", "선임시기", "비고"]), "회사명"),
        ("우리금융지주(구)·회사먼저형",
         _tbl(["회사명", "직책", "성명", "선임일자", "비고"]), "회사명"),
    ]:
        check("겸직 머리행 판별: %s" % _name,
              config.interlock_header(_t, _N) == _want,
              "기대 %r, 관측 %r" % (_want, config.interlock_header(_t, _N)))

    # (2) 실측된 오탐 둘은 반드시 버려야 한다. 사람칸·직위칸이 없다.
    check("겸직 오탐 제외: KB국민카드 '시기|대상회사|주요 내용'",
          config.interlock_header(
              _tbl(["시기", "대상회사", "주요 내용 및 재무제표에 미치는 영향",
                    "공시서류 제출 등"]), _N) == "",
          "기타 재무 표가 겸직으로 잡혔다")
    check("겸직 오탐 제외: 메리츠 '구분|내역|대상회사|한도금액'",
          config.interlock_header(
              _tbl(["구분", "내역", "대상회사", "한도금액", "사용금액"]), _N) == "",
          "지급보증 한도 표가 겸직으로 잡혔다")
    check("겸직 오탐 제외: '회사명' 단독 (실측 수천 건)",
          config.interlock_header(_tbl(["회사명", "업종", "자산총액", "지분율"]), _N) == "",
          "회사명만 있는 표가 겸직으로 잡혔다 — 조합 조건이 풀렸다")

    # (3) 위원회 구성표. 「위원회명」 한 칸이 여섯 양식을 모두 가른다.
    for _name, _t in [
        ("KB·우리·iM·한국", _tbl(["위원회명", "구성", "소속 이사명",
                                  "설치목적 및 권한사항", "비고"])),
        ("신한지주", _tbl(["위원회명", "구성 및 소속이사명", "설치목적 및 권한사항", "비 고"])),
        ("메리츠금융지주", _tbl(["구분", "위원회명", "구성", "소속 이사명",
                                 "설치목적 및 권한사항", "비 고"])),
    ]:
        check("위원회 머리행 판별: %s" % _name,
              config.committee_header(_t, _N) == "위원회명")
    check("위원회 오탐 제외: 이사회 안건 표",
          config.committee_header(
              _tbl(["회차", "개최일자", "의안내용", "가결여부"]), _N) == "",
          "안건 표가 위원회 구성표로 잡혔다")

    # (4) 머리행 규칙은 **한 군데**에만 적혀 있어야 한다. emit 이 표를 전개할지와
    #     handoff5 가 표를 고를지가 다른 목록을 보면 4차 때처럼 조용히 어긋난다.
    check("handoff5 가 config.interlock_header 를 그대로 쓴다",
          _h5.INTERLOCK_HEADER is config.interlock_header,
    ...
    check("임원표 오탐 제외: 투자운용인력현황(상세)",
          config.officer_header(
              _tbl(["성명", "직위", "담당업무", "자격증종류", "자격증취득일",
                    "주요경력", "협회등록일자", "상근여부", "운용중인계약수"]), _N) == "",
          "펀드 운용인력 510명이 KB금융 임원으로 섞였던 자리다")

    # (5) 5차 문서 집합은 **접수번호 목록**이다. 규칙(사업연도)으로 적으면 다른 법인의
    #     같은 연도 사업보고서까지 좁은 키워드 세트로 바뀌어 1~3차 산출물이 줄어든다.
    check("DOC_5CHA 는 10건이고 전부 14자리 접수번호",
          len(config.DOC_5CHA) == 10
          and all(len(k) == 14 and k.isdigit() for k in config.DOC_5CHA),
          "관측 %d건" % len(config.DOC_5CHA))
    check("메리츠 설립 첫해(3월 결산) 2건이 목록에 있다",
          {"20110629000474", "20120629000621"} <= set(config.DOC_5CHA),
          "12월 결산을 전제하면 '(2011.03)' 을 놓친다")
    check("우리금융지주 FY2019 는 DOC_5CHA 가 아니다(머리행만 추가)",
          not config.is_5cha_doc("20200330004490")
          and "20200330004490" in config.DOC_5CHA_HEADER_ONLY,
          "좁은 키워드 세트로 바뀌면 이미 있던 행이 줄어든다 — 그건 삭제다")

    # (6) 머리행 규칙 적용 문서. 1~3차 문서에 적용되면 기존 산출물이 바뀐다.
    check("머리행 규칙: 4차 문서에 적용",
          config.is_header_rule_doc("사업보고서 (2024.12)", "20250318", "20250318000993"))
    check("머리행 규칙: DOC_5CHA 에 적용",
          config.is_header_rule_doc("사업보고서 (2019.12)", "20200330", "20200330003956"))
    check("머리행 규칙: 우리금융지주 FY2019 에 적용",
          config.is_header_rule_doc("사업보고서 (2019.12)", "20200330", "20200330004490"))
    check("머리행 규칙: 1~3차 문서에는 적용하지 않는다",
          not config.is_header_rule_doc("증권신고서(주식의포괄적교환ㆍ이전)",
                                        "20181203", "20181203000123"),
          "적용하면 비4차 문서에서 위원회 253표·겸직 14표가 새로 열려 1~3차가 바뀐다")

    # (7) 신규 5곳은 문서 종류를 가리지 않고 전량 5차다. 문서 규칙만 보면 그 법인들의
    #     FY2023~25 사업보고서가 4차로 가서 4차 산출물의 뜻이 바뀐다.
    check("신규 5곳의 FY2024 사업보고서는 4차가 아니다",
          not _h._is_4cha_row({"corp_label": "하나금융지주",
                               "report_nm": "사업보고서 (2024.12)", "rcept_dt": "20250318"}),
          "4차 10곳짜리 산출물에 법인 5곳이 말없이 섞인다")
    check("신규 5곳의 FY2024 사업보고서는 5차다",
          _h._is_5cha_row("20250318009999", {"corp_label": "하나금융지주"}))
    check("기존 법인의 FY2024 사업보고서는 여전히 4차다",
          _h._is_4cha_row({"corp_label": "KB금융", "report_nm": "사업보고서 (2024.12)",
                           "rcept_dt": "20250318"}),
          "4차 배치가 깎이면 기존 산출물이 줄어든다")
    check("5차가 BATCHES 에 있다", "5차" in _h.BATCHES
          and "5차" in _h.NARRATIVE_NAME and "5차" in _h.TABLE_NAME)

    # (8) 현직 판별 패턴. 회사마다 표기가 달라 '~현재' 만 보면 놓친다.
    import re as _re
    _pats = dict(config.CURRENT_POSITION_PATTERNS)
    check("현직 판별 패턴 5종이 전부 컴파일된다",
          len(config.CURRENT_POSITION_PATTERNS) == 5
          and all(_re.compile(v) for v in _pats.values()))
    check("현직 판별: '2023.07~현재' 가 잡힌다",
          bool(_re.search(_pats["~현재"], "신한은행 부행장 겸직, 2023.07~현재")))
    check("현직 판별: '(現)' 가 잡힌다", bool(_re.search(_pats["(現)"], "(現) 우리은행 부행장")))
    check("현직 판별: '재직중' 이 잡힌다", bool(_re.search(_pats["재직중"], "하나은행 부행장 재직 중")))
